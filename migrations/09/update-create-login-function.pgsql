...
drop function epubtest.create_new_login(text, text);
drop function epubtest.create_new_login(text, text, boolean);
create or replace function epubtest.create_new_login(email text, pwd text, active boolean)
returns int
 as $$
declare
    new_id integer;
...
begin
    ...
    if not found then 
        ...
        values (create_new_login.email, crypt(create_new_login.pwd, gen_salt('bf')), active)
